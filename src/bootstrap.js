...
function findIncludeUserCheckbox() {
    // 1) ラベルのテキストから候補を拾う
    ...
    // 2) 属性フォールバック
    checkbox = document.querySelector('input[type="checkbox"][name*="user" i], input[type="checkbox"][id*="user" i], input[type="checkbox"][data-testid*="user" i]');
    return checkbox ? { checkbox } : null;
}
/** “ユーザー操作に近い”手順でONにする */
function turnOnBySimulatedUserAction(checkbox, label) {
    if (checkbox.checked)
        return; // 既にONなら何もしない
